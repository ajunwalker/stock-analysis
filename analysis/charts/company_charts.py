...
class CompanyCharts:
    ...
    def show_revenue_estimates(company: Company):
        ...
        df = pd.DataFrame(
            dict(dates=dates, revenue=revenue, color=color_discrete_sequence)
        )
    ...
    @staticmethod
    async def get_peer_charts(symbol: str) -> None:
        peers = [symbol] + CompanyAPI.get_peers(symbol)
        prices = CompanyAPI.get_bulk_prices(peers)

        rows, remainder = divmod(len(peers), 2)
        if remainder == 1:
            rows += 1

        titles = [CompanyCharts.build_chart_title(price) for price in await prices]
        fig = make_subplots(
            rows=rows, cols=2, subplot_titles=titles, vertical_spacing=0.15
        )

        for idx, peer in enumerate(peers):
            row, col = divmod(idx, 2)

            chart_df = await CompanyAPI.get_daily_chart(peer)
            fig.add_trace(
                go.Scatter(x=chart_df["date"], y=chart_df["adjClose"]),
                row=row + 1,
                col=col + 1,
            )
    ...
    def show_key_metrics(company: Company) -> None:
        ...
        fig.add_trace(
            go.Bar(x=company.ratios["date"], y=company.ratios["priceEarningsRatio"]),
            row=1,
            col=1,
        )
        fig.add_trace(
            go.Bar(x=company.ratios["date"], y=company.ratios["priceToBookRatio"]),
            row=1,
            col=2,
        )
