...
class ScreenerAPI(BaseAPI):
	...
	async def get_company_profiles(symbols: list[str]):
		...
		for company in profiles:
			compiled.append({
				...
				"Revenue Change (%)": CompanyData.get_last_revenue_percentage_change(company),
				"P/E": CompanyData.get_last_ratio_value(company, "priceEarningsRatio"),
				"P/B": CompanyData.get_last_ratio_value(company, "priceToBookRatio"),
	...
	async def search(
		...
		for idx in range(0, len(symbols), window):
			response = await ScreenerAPI.get_company_profiles(symbols[idx: idx + window])
			responses.extend(response)

		profiles_df = pd.DataFrame(responses)
		date_before = (datetime.date.today() - datetime.timedelta(days=7 * 52 * 5)).strftime('%Y-%m-%d')
		...
		if revenue_change_more_than:
			profiles_df = profiles_df.loc[profiles_df["Revenue Change (%)"] > revenue_change_more_than]
