...
class CompanyData:
	...
	@staticmethod
	def get_percentage_change(new_value: int, old_value: int):
		change = (new_value - old_value) / old_value
		return round(change * 100, 2)

	@staticmethod
	def get_daily_change(company: Company, days: int) -> float:
	...
	def get_last_revenue_change(company: Company) -> str:
		if "revenue" in company.income:
			return format_number(company.income["revenue"].iloc[0] - company.income["revenue"].iloc[1])
		return "N/A"
	...
	def get_last_revenue_percentage_change(company: Company) -> float:
		if "revenue" in company.income:
			change = CompanyData.get_percentage_change(
				company.income["revenue"].iloc[0],
				company.income["revenue"].iloc[1]
			)
			return change
		return float('-inf')
	...
	def print_profile(company: Company) -> None:
		...
		kwargs = {
			...
			"cash": format_number(company.balance_sheet.iloc[0]["cashAndCashEquivalents"]),
	...
	def get_company_cash_runway(company: Company) -> pd.DataFrame:
		...
		for idx1 in range(min_len - 3):
			...
			for idx2 in range(4):
				trailing_free_cashflow += company.cashflow.iloc[idx1 + idx2]["freeCashFlow"]
